...
# Particiapnts is from 2024 summer - so number of athletes will differ
...
print("Medal data saved to medals.csv")
...
for country in country_arrays.keys():
    ...
    ax = sns.heatmap(arr, annot=False, fmt=".0f", cmap="viridis", cbar=False,
        norm=matplotlib.colors.LogNorm( vmin=1, vmax=max(len(country_arrays), len(country_arrays)-20)))
    ax.invert_yaxis()
